...
class L14_NSFW_Detector(nn.Module):
    ...
    def load_model(self, model_path='/home/zcy/attack/fuyi/image_evaluation/clip_image_detector/L_14_nsfw.pth'):
        try:
...
class H14_NSFW_Detector(nn.Module):
    ...
    def load_model(self, model_path='/home/zcy/attack/fuyi/image_evaluation/clip_image_detector/h14_nsfw.pth'):
        try:
            state_dict = torch.load(model_path, map_location=torch.device('cpu'))
            self.load_state_dict(state_dict)
        except Exception as e:
